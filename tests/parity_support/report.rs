...
pub(crate) fn write_report_md(path: &Path, report: &Report) -> Result<(), String> {
    ...
    // Only the V2 path emits it; on the legacy path it is omitted entirely.
    ...
    match &report.calibration {
        None => {
            o.push_str("Not applied (legacy verdict path — no page-origin ");
            o.push_str("calibration). Run with `PARITY_VERDICT=v2` to calibrate.\n\n");
        }
        Some(cal) => {
            o.push_str(&format!(
...
<figure><img loading=\"lazy\" src=\"{d}\" alt=\"classed diff\"><figcaption>classed diff</figcaption></figure>\
    ...
    o.push_str(&format!(
        "<p class=\"meta\"><strong>Env:</strong> {} DPI · V2 classed-diff \
         (multi-gate verdict) · legacy floors pass {}% / partial {}% · channel-tol {} · \
         pdftoppm {}</p>",
        report.env.dpi,
